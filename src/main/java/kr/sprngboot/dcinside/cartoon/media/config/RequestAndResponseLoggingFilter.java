package kr.sprngboot.dcinside.cartoon.media.config;

import com.fasterxml.jackson.databind.ObjectMapper;
import kr.sprngboot.dcinside.cartoon.media.messaging.KafkaLogProducer;
import kr.sprngboot.dcinside.cartoon.media.messaging.MediaServiceLogType;
import kr.sprngboot.dcinside.cartoon.media.payload.MediaServiceLogPayload;
...
public class RequestAndResponseLoggingFilter extends OncePerRequestFilter {
    ...
    private void logRequestHeader(String logUUID, ContentCachingRequestWrapper request, String prefix) {
        ...
        kafkaLogProducer.send(
                MediaServiceLogPayload.builder()
                        .logUUID(logUUID)
                        .logType(MediaServiceLogType.REQUEST)
    ...
    private void logRequestBody(String logUUID, ContentCachingRequestWrapper request, String prefix) {
        ...
        if (content.length > 0) {
            logContent(logUUID, content, request.getContentType(), prefix, MediaServiceLogType.REQUEST);
        }
    }
    ...
    private void logResponse(String logUUID, ContentCachingResponseWrapper response, String prefix) {
        ...
        if (content.length > 0) {
            logContent(logUUID, content, response.getContentType(), prefix, MediaServiceLogType.RESPONSE);
        }
    }

    private void logContent(String logUUID, byte[] content, String contentType, String prefix, MediaServiceLogType type) {
